...
REST.prototype.delAll = function(req, res, next, params){
  var db = req.params.db,
      ...
      ops = [];
  if (!this.running) { return this.error('stopping', req, res, next) }
  this.db.getStream(db, utils.extend(req.params, {values: false}), function(err, data){
    if (err) { return this.error(err, req, res, next) }
    for (var i in data) { // remember, data is an array coz we got no values
      ops.push({type: 'del', key: data[i]});
    }
    this.db.batch(db, ops, function(err){
      if (err) { return this.error(err, req, res, next) }
      this.respond({deleted: data}, req, res, next);
    }.bind(this));
  }.bind(this));
};

...
var DEFAULT_CONTENT_TYPE = 'application/json',
    ...
    ROUTES = {
      ...
      del:      ['DEL',   'db/:dbName/:key'],
      delRange: ['DEL',   'db/:dbName/:fromKey/:toKey'],
...
REST.prototype.del = function(req, res, next){
  var dbName = req.params.dbName,
      key = req.params.key;
  if (!this.running) { return this.error('stopping', req, res, next) }
  this.db.del(dbName, key, function(err, result){
    if (err) { return this.error(err, req, res, next) }
    this.respond({db: dbName, deleted: key}, req, res, next);
  }.bind(this));
};
...
REST.prototype.delRange = function(req, res, next){
  var dbName = req.params.dbName,
      ...
      ops = [];
  if (!this.running) { return this.error('stopping', req, res, next) }
  this.db.getStream(dbName, {gte: fromKey, lte: toKey, values: false}, function(err, data){
    if (err) { return this.error(err, req, res, next) }
    for (var i in data) {
      ops.push({type: 'del', key: data[i]});
    }
    this.db.batch(dbName, ops, function(err){
      if (err) { return this.error(err, req, res, next) }
      this.respond({db: dbName, fromKey: fromKey, toKey: toKey, deleted: data}, req, res, next);
    }.bind(this));
  }.bind(this));
};
